...
function CreateOrderContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // setTimeout(() => {
      //   window.location.href = `/order/${result.orderNumber}`;
      // }, 50000); // Changed from 50000 to 5000 milliseconds (5 seconds)

      // In your handleSubmit function, replace the setTimeout with:
      if (result.orderNumber) {
        setOrderNumber(result.orderNumber);
        setSuccess(true);

        // Set a timer to show the redirect button after 50 seconds
        setTimeout(() => {
        ...
        <div className={styles.successMessage}>
          ...
          {redirectReady && (
            <button
              ...
              onClick={() => (window.location.href = `/order/${orderNumber}`)}
            >
              View Order Details
            </button>
          )} */}
...
export default function CreateOrderPage() {
  return (
    <Suspense
      fallback={
        <div className={styles.loadingContainer}>Loading order page...</div>
      }
    >
      <CreateOrderContent />
    </Suspense>
